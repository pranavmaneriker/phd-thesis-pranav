...
\begin{figure}
    \begin{subfigure}{0.48\linewidth}
        \includegraphics[width=\linewidth]{graphConformal/figures/nspc/amazon_photos_10_size_stratified_coverage.pdf}
    \end{subfigure}
    \begin{subfigure}{0.48\linewidth}
        \includegraphics[width=\linewidth]{graphConformal/figures/nspc/amazon_photos_40_size_stratified_coverage.pdf}
    \end{subfigure}
...
\end{figure}
Figure~\ref{fig:fs:conformal:aps_vs_randomized} and Figure~\ref{fig:nspc:conformal:aps_vs_randomized} provide violin plots that compare the efficiency of randomized and non-randomized version of APS across different datasets and $\alpha$.
...
\begin{table}[h]
    ...
    \begin{tabular}{llll}
        ...
        dataset &  &  &  \\
        \midrule
        CiteSeer & 188.07 $\pm$ 1.86 & 8.33 $\pm$ 0.10 & 4.46 $\pm$ 0.03 \\
        Amazon\_Photos & 291.41 $\pm$ 1.54 & 29.68 $\pm$ 0.43 & 8.29 $\pm$ 0.12 \\
        Cora & 986.64 $\pm$ 22.80 & 89.66 $\pm$ 0.60 & 29.22 $\pm$ 0.94 \\
        PubMed & 253.97 $\pm$ 2.19 & 58.26 $\pm$ 0.48 & 15.47 $\pm$ 0.11 \\
        Coauthor\_CS & 665.98 $\pm$ 15.29 & 72.80 $\pm$ 0.46 & 17.92 $\pm$ 0.87 \\
        Coauthor\_Physics & 2080.58 $\pm$ 31.44 & 758.00 $\pm$ 4.27 & 27.71 $\pm$ 0.54 \\
        \bottomrule
    \end{tabular}
    \caption{Runtime improvements in CFGNN implementation starting from the baseline, then adding batching, and then adding caching and batching combined. For each setup we compare the results from 5 runs and provide a $[0.2, 0.8]$ quantile interval in the reported results. All runtimes in seconds, runs executed on a single A100 GPU.}
...
We note that our implementation is able to achieve improvements ranging from 16.4x (PubMed) to 75x (Coauthor\_Physics) in runtime over the baseline implementation.
